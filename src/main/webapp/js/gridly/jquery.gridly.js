...
(function() {
  ...
  Gridly = (function() {
    ...
    function Gridly($el, settings) {
      ...
      this.currentElements = null;//SmileEdit
      this.$el = $el;
    ...
    Gridly.prototype.draggingBegan = function(event) {
      ...
      this.currentElements = $elements; //SmileEdit
      setTimeout(this.layout, 0);
    ...
    Gridly.prototype.draggingEnded = function(event) {
      ...
      //SmileEdit start
      if (isNotValidChanges(this.currentElements, $elements, this._draggable.dragged)) {
        this.ordinalize(this.currentElements);
        return void 0;
      }
      //SmileEdit end

        return (ref = this.settings) != null ? (ref1 = ref.callbacks) != null ? typeof ref1.reordered === "function" ? ref1.reordered(this.currentElements, $elements) : void 0 : void 0 : void 0;
    };

    //SmileEdit start

    function isNotValidChanges(currentElements, newElements, dragged) {
        for (var i = currentElements.length - 1; i >= 0; i--) {
            var $currElem = $(currentElements[i]);
            if($currElem.hasClass("nonDraggable") && isNotEqualBlocks($currElem, $(newElements[i]))) {
                return true;
            }
        }
        return $(dragged).hasClass("nonDraggable");
    }

    function isNotEqualBlocks($firstElem, $secondElem) {
        return $firstElem.attr("data-portletInstanceId") !== $secondElem.attr("data-portletInstanceId");
    }
